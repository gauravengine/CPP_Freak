	...
	while (cin >> n) {
		...
		while (j < n) {
			...
			while (row < n && col < n) {
				...
				for (; k < col; k++) {
					int temp = cum[k];
					if (row > 0) temp -= cum[row - 1];
					temp = temp % 100;
					int temp2 = (cum[col] - cum[k]) % 100;

					dp[row][col] = min(dp[row][col], dp[row][k] + dp[k + 1][col] + temp * temp2);
				}
