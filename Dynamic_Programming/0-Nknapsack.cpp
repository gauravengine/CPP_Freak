...
void print(vec<1,int> &wt,vec<1,int> &val){
	for(int i=0;i<val.size();i++){
		cout<<wt[i]<<" : "<<val[i]<<'\n';
	}
}
vec<2,int> dp(1005,1005,-1);

int knapsack(int i,int c){
	if(i>=wt.size()) return 0;
	if(dp[i][c]!=-1) return dp[i][c];
	int op1=0;
	if(c>=wt[i]) op1=val[i]+knapsack(i+1,c-wt[i]);
	int op2=knapsack(i+1,c);

	return dp[i][c]=max(op2,op1);
}
...
void solve(){
    ...
    cin>>n>>x;
    cap=x;
    int tmpwt[n],tmpval[n];
    for(int i=0;i<n;i++) cin>>tmpwt[i];
    for(int i=0;i<n;i++) cin>>tmpval[i];
    unordered_map<int,int> hash;
    for(int i=0;i<n;i++){
    	int temp=tmpwt[i];
    	int v=tmpval[i];
    	int tempv=v;
    	hash[temp]=max(hash[temp],v);
    	for(int j=temp;j<=cap;j+=temp,v+=tempv){
    		//db2(j,v);
    		hash[j]=max(hash[j],v);
    	}
    }
