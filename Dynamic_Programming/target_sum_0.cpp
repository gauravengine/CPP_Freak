...
bool help(int arr[],int curr,int n,int sum,bool chosen){
	if(curr==n){
		if(sum==0 && chosen) return true;
		else return false;
	}
	//if(sum==0) return true;
	bool op1=help(arr,curr+1,n,sum+arr[curr],true);
	bool op2=help(arr,curr+1,n,sum,chosen|false);
	return op1|op2;
}
