...
void dijkstra(int src,int n, vec<1,int> &dis,vec<1,pii> adj[],vec<1,int> &prev){
	//fill dis with 1LL<<61
...
void solve(){
	...
	vec<1,int> dis(n+1,1LL<<61),prev(n+1,-1);
	dis[0]=0;
	//dis[1]=0;
	dijkstra(1,n,dis,adj,prev);
	// print dis
	for(int i=1;i<=n;i++){
		cout<<dis[i]<<" ";
	}
