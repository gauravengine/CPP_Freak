...
void solve() {
	...
	for (int len = 1; len <= 3; len++) {
		if(len==1)for (int i = 0; i < 26; i++) ss.insert( string(1, (char)(i + 'a')) );
		else if(len==2)for (int i = 0; i < 26; i++)for (int j = 0; j < 26; j++)ss.insert(string(1, (char)(i + 'a')) + string(1, (char)(j + 'a')) );
		else for (int i = 0; i < 26; i++)for (int j = 0; j < 26; j++)for (int k = 0; k < 26; k++)ss.insert(string(1, (char)(i + 'a')) + string(1, (char)(j + 'a')) + string(1, (char)(k + 'a')) );
		for (int i = 0; i + len <= n; i++) {
			string temp = s.substr(i, len);
			if (ss.count(temp))  ss.erase(temp);
		}
		if(!ss.empty()){
			ans=*(ss.begin());
			break;
